...
class Register(db.Model):
    ...
    cnicnumber=db.Column(db.String(50))
...
def edit(rid):
    ...
    if request.method=="POST":
        ...
        # query=db.engine.execute(f"UPDATE `register` SET `farmername`='{farmername}',`cnicnumber`='{cnicnumber}',`age`='{age}',`gender`='{gender}',`phonenumber`='{phonenumber}',`address`='{address}',`farming`='{farmingtype}'")
        post=Register.query.filter_by(rid=rid).first()
        print(post.farmername)
        post.farmername=farmername
        post.cnicnumber=cnicnumber
        post.age=age
        post.gender=gender
...
def register():
    ...
    if request.method=="POST":
        ...
        address=request.form.get('address')
        farmingtype=request.form.get('farmingtype')     
        query=Register(farmername=farmername,cnicnumber=cnicnumber,age=age,gender=gender,phonenumber=phonenumber,address=address,farming=farmingtype)
        db.session.add(query)
        db.session.commit()
        # query=db.engine.execute(f"INSERT INTO `register` (`farmername`,`cnicnumber`,`age`,`gender`,`phonenumber`,`address`,`farming`) VALUES ('{farmername}','{cnicnumber}','{age}','{gender}','{phonenumber}','{address}','{farmingtype}')")
